...
class deneme8 : AppCompatActivity() {
    private lateinit var auth: FirebaseAuth
    private lateinit var database: FirebaseFirestore
    private lateinit var recyclerViewAdapter: tariffeedadapter2_8
    var tariflistesi = ArrayList<tarif8>()
    //val displayList = ArrayList<tarif>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_deneme8)
        ...
        recyclerViewAdapter = tariffeedadapter2_8(tariflistesi)
        ...
        exitbutton1.setOnClickListener {
            val intent = Intent(this@deneme8, MainActivity::class.java)
            startActivity(intent)
            finish()
        }
       commentbutton_2.setOnClickListener {
            val intent = Intent(this@deneme8, tarifekle8::class.java)
            startActivity(intent)
            finish()
        }
                    ...
                    if (snapshot != null) {
                        if (!snapshot.isEmpty) {
                            ...
                            for (document in documents) {
                                ...
                                val indirilenpost1 = tarif8(tarifadi,kullaniciemaili,tarifkategori,tarifmalzemeler,tarifmalzemeolculeri,tarifinstructions,x)
